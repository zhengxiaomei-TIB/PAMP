...
For spore and mycelium images, you can use the **online Segment Anything tool** for segmentation:

🔗 **Access the tool**: https://huggingface.co/spaces/Xenova/segment-anything-web

**Steps:**

1. **Access the online service** - Open the above link in your web browser
2. **Upload images** - Upload your spore or mycelium images directly to the web interface
3. **Perform segmentation** - Use the tool's manual or automatic segmentation features to accurately segment the target areas
4. **Download results** - Download the segmented images for further processing
...
```bash
python PAMP.py --input_folder ./data/input/ --output_folder ./data/output/
```
...
|-----------|-------------|---------------|
| `--input_folder` | Input folder path | `./data/input/` |
| `--output_folder` | Output folder path | `./data/output/` |  
...
- **CSV File**: `/data/output/Pellets/Pellets results.csv`, containing the following parameters:
...
- **CSV Files**:
  - `/data/output/Spores/Spores results.csv`: Spore analysis results
  - `/data/output/Hyphae/Hyphae results.csv`: Mycelium analysis results
...
```bash
python PAMP.py --input_folder ./data/input/ --output_folder ./data/output/ --Flat_field_img ./data/input/Flat_field.jpg
```

### Adjust Scale Factors

```bash
python PAMP.py --input_folder ./data/input/ --output_folder ./data/output/ --pellet_scale 2.0 --hyphae_scale 0.3
```
